...
export default function Blogs() {
  ...
  useEffect(() => { fetch(API_URL).then((response) => { if (!response.ok) throw new Error(); return response.json(); }).then(setBlogs).catch(() => setError("We could not load the blogs. Please try again shortly.")).finally(() => setLoading(false)); }, []);
  return <div className="min-h-screen bg-[#050b14] text-white"><Navbar /><main className="px-5 pb-24 pt-36"><section className="mx-auto max-w-7xl">
    ...
    {!loading && !error && <div className="grid gap-7 md:grid-cols-2 lg:grid-cols-3">{blogs.map((blog, index) => <motion.article key={blog._id} initial={{ opacity: 0, y: 28 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * .06 }} className="group overflow-hidden rounded-3xl border border-white/10 bg-white/[.035] transition hover:-translate-y-2 hover:border-cyan-400/40">{blog.image ? <img src={blog.image} alt={blog.title} className="h-52 w-full object-cover transition duration-500 group-hover:scale-105" /> : <div className="h-52 bg-gradient-to-br from-cyan-500/20 to-blue-600/20" />}<div className="p-6"><div className="flex items-center justify-between gap-3 text-xs text-gray-400"><span className="rounded-full bg-cyan-500/10 px-3 py-1 text-cyan-300">{blog.category || "General"}</span><span className="flex items-center gap-1"><FiCalendar /> {new Date(blog.createdAt).toLocaleDateString()}</span></div><h2 className="mt-5 text-2xl font-bold transition group-hover:text-cyan-300">{blog.title}</h2><p className="mt-3 line-clamp-3 leading-7 text-gray-400">{blog.excerpt}</p><Link to={`/blogs/${blog.slug}`} className="mt-6 inline-flex items-center gap-2 font-semibold text-cyan-400">Read article <FiArrowRight /></Link></div></motion.article>)}</div>}
