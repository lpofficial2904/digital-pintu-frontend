...
// Modern & recommended over react-helmet
import {
...
export default function ServiceDetails() {
  ...
  const commonCopy = contentSettings.serviceDetails;
  ...
  const pageUrl = service.canonicalUrl || window.location.href;
  const serviceSchema = { "@context":"https://schema.org", "@type":"Service", name:service.title, description:metaDescription, url:pageUrl, image:service.ogImage || service.image || undefined, areaServed:{ "@type":"City", name:"Jaipur" }, provider:{ "@type":"ProfessionalService", name:"Digital Pintu", url:"https://www.digitalpintu.com", address:{ "@type":"PostalAddress", addressLocality:"Jaipur", addressRegion:"Rajasthan", addressCountry:"IN" } } };
  const breadcrumbSchema = { "@context":"https://schema.org", "@type":"BreadcrumbList", itemListElement:[{ "@type":"ListItem", position:1, name:"Home", item:"https://www.digitalpintu.com/" },{ "@type":"ListItem", position:2, name:"Services", item:"https://www.digitalpintu.com/services" },{ "@type":"ListItem", position:3, name:service.title, item:pageUrl }] };
  const faqSchema = service.faqs?.length ? { "@context":"https://schema.org", "@type":"FAQPage", mainEntity:service.faqs.map((faq)=>({ "@type":"Question", name:faq.question, acceptedAnswer:{ "@type":"Answer", text:faq.answer } })) } : null;
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#050b14] pb-24 pt-36 text-white">
      ...
      <Helmet>
        ...
        <meta name="robots" content={service.noIndex ? "noindex,follow" : "index,follow"} />
        {service.metaKeywords && <meta name="keywords" content={service.metaKeywords} />}
        <link rel="canonical" href={pageUrl} />

        {/* Open Graph / Facebook */}
        <meta property="og:type" content="website" />
        <meta property="og:title" content={service.ogTitle || siteTitle} />
        <meta property="og:description" content={service.ogDescription || metaDescription} />
        <meta property="og:url" content={pageUrl} />
        {(service.ogImage || service.image) && <meta property="og:image" content={service.ogImage || service.image} />}

        {/* Twitter */}
        <meta name="twitter:card" content="summary_large_image" />
        <meta name="twitter:title" content={service.ogTitle || siteTitle} />
        <meta name="twitter:description" content={service.ogDescription || metaDescription} />
        {(service.ogImage || service.image) && <meta name="twitter:image" content={service.ogImage || service.image} />}
        <script type="application/ld+json">{JSON.stringify(serviceSchema)}</script><script type="application/ld+json">{JSON.stringify(breadcrumbSchema)}</script>{faqSchema && <script type="application/ld+json">{JSON.stringify(faqSchema)}</script>}
      </Helmet>
        ...
        <motion.div initial="hidden" animate="visible" variants={staggerContainer}>
          ...
          <div className="mt-8 grid items-center gap-12 lg:grid-cols-2">
            ...
            <motion.div variants={fadeInUp} className="overflow-hidden rounded-3xl border border-white/10 bg-white/[0.02] shadow-2xl">
              {service.image ? (
                <img src={service.image} alt={service.imageAlt || service.title} className="h-full max-h-[420px] w-full object-cover transition-transform duration-700 hover:scale-105" />
              ) : (
        ...
        {highlights.length > 0 && (
          ...
          >
            <div className="text-center max-w-3xl mx-auto mb-16">
              <h2 className="text-3xl font-bold md:text-4xl">{commonCopy.featuresHeading}</h2>
              <p className="mt-4 text-gray-400">{commonCopy.featuresDescription}</p>
            </div>
        ...
        {technologies.length > 0 && (
          ...
          >
            <div className="text-center max-w-2xl mx-auto mb-16">
              <h2 className="text-3xl font-bold md:text-4xl">{commonCopy.technologiesHeading}</h2>
              <p className="mt-4 text-gray-400">{commonCopy.technologiesDescription}</p>
            </div>
